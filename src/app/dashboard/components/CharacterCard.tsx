...
export default function CharacterCard({ char }: { char: any }) {
  ...
  return (
    <div className="relative group rounded-xl bg-parchment-900/40 border border-white/5 hover:border-blood-500/50 transition-all">
      ...
      <Link href={`/dashboard/characters/${char.id}`} className="flex flex-col flex-1 cursor-pointer p-5 group-hover:bg-white/5 transition-colors rounded-t-xl">
        <div className="flex items-start gap-4 mb-2 w-full">
          ...
          <div className="flex flex-col flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              <h3 className="font-bold text-white group-hover:text-amber-400 transition-colors truncate text-lg">{char.name}</h3>
              <button 
            ...
            {campaign && (
              <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-2">
                ...
                <span className="text-[10px] text-emerald-400/80 truncate uppercase tracking-widest font-semibold">{campaign.title}</span>
              </div>
            )}
          </div>
        </div>
